...
# mixture model-based clustering ---- k = 10  -------------
library(mclust)
...
# Affinity Propagation clustering ---- k = 28  -------------
require(apcluster)
...
currentNumberOfClusters <- currentNumberOfClusters + 1   #update current number of clusters
...
divClustResults <- cutree(as.hclust(divClust2), h = 0.27)
table(divClustResults)
...
divClustResults <- cutree(as.hclust(divClust), h = 0.31)
table(divClustResults)
clusterMembership <- c(clusterMembership, (divClustResults + currentNumberOfClusters)) 
...
divClustResults <- cutree(as.hclust(divClust), h = 0.34)
table(divClustResults)
clusterMembership <- c(clusterMembership, (divClustResults + currentNumberOfClusters)) 
currentNumberOfClusters <- currentNumberOfClusters + 4
...
divClustResults <- cutree(as.hclust(divClust), h = 0.35)
table(divClustResults)
clusterMembership <- c(clusterMembership, (divClustResults + currentNumberOfClusters))
